...
Countly.sendEvent = function(options) {
    ...
    if (options.eventName) args.push(options.eventName.toString());
...
Countly.sendPushToken = function(options) {
    ...
    args.push(
        options.messagingMode.toString() ||
            Countly.messagingMode.PRODUCTION.toString()
    );
    cordova.exec(
        Countly.onSuccess,
        Countly.onError,
    ...
    if (Array.isArray(exception)) {
        for (var i = 0, il = exception.length; i < il; i++) {
            if (typeof exception[i] === "string") {
                ...
                exceptionString +=
                    "columnNumber: " + exception[i].columnNumber + "\n";
                exceptionString += "fileName: " + exception[i].fileName + "\n";
                exceptionString +=
                    "functionName: " + exception[i].functionName + "\n";
                exceptionString +=
                    "lineNumber: " + exception[i].lineNumber + "\n";
            }
        }
...
Countly.endEvent = function(options) {
    if (typeof options === "string") options = { eventName: options };
    var args = [];
    var eventType = "event"; //event, eventWithSum, eventWithSegment, eventWithSumSegment
    var segments = {};
    ...
    if (!options.eventSum) options.eventSum = "0";
    args.push(options.eventSum.toString());
...
Countly.userData.setProperty = function(keyName, keyValue) {
    cordova.exec(
        ...
        [keyName.toString() || "", keyValue.toString() || ""]
    );
};
...
Countly.userData.incrementBy = function(keyName, keyIncrement) {
    cordova.exec(
        ...
        [keyName.toString() || "", keyIncrement.toString() || ""]
    );
};
Countly.userData.multiply = function(keyName, multiplyValue) {
    cordova.exec(
        ...
        "CountlyCordova",
        "userData_multiply",
        [keyName.toString() || "", multiplyValue.toString() || ""]
    );
};
Countly.userData.saveMax = function(keyName, saveMax) {
    cordova.exec(
        ...
        "CountlyCordova",
        "userData_saveMax",
        [keyName.toString() || "", saveMax.toString() || ""]
    );
};
Countly.userData.saveMin = function(keyName, saveMin) {
    cordova.exec(
        ...
        "CountlyCordova",
        "userData_saveMin",
        [keyName.toString() || "", saveMin.toString() || ""]
    );
};
Countly.userData.setOnce = function(keyName, setOnce) {
    cordova.exec(
        ...
        "CountlyCordova",
        "userData_setOnce",
        [keyName.toString() || "", setOnce.toString() || ""]
    );
};
...
Countly.rating.closeModal = function() {
    ...
    if (Countly.rating.callback) {
        Countly.rating.callback({
            code: 1,
            msg: "The star rating dialog was dismissed."
        });
    }
};
...
Countly.rating.html =
    '<div class="countly-modal-header"></div><div class="countly-modal-content"></div><div class="countly-modal-divider"></div><div class="countly-modal-dismiss"></div><div class="countly-modal-submit">Submit</div>';
Countly.rating.css =
...
Countly.rating.create = function() {
    ...
    query("countly-modal-dismiss").addEventListener(
        "click",
        Countly.rating.closeModal
    );
    query("countly-modal-submit").addEventListener(
        "click",
        Countly.rating.send
    );
};
Countly.rating.set = function(rating) {
    ...
    for (var i = 1; i < 6; i++) {
        html +=
            '<span class="countly-rating-start" data-rating="' +
            i +
            '">' +
            (rating >= i ? "&#x2605" : "&#x2606;") +
            "</span>";
    ...
    query("countly-modal-dismiss").innerText =
        Countly.rating.starRatingDismissButtonTitle;
    query("countly-modal-submit").innerText =
        Countly.rating.starRatingSubmitButtonTitle;
