...
class NetworkRest extends EventEmitter {
   ...
   async _request(params, jobResponse, numRetries = 1) {
      ...
      if (!navigator.onLine) {
         await this.queue(params, jobResponse);
   ...
   async get(params, jobResponse) {
      ...
      response.status !== "success" && (await this.queue(params, jobResponse));
   ...
   async queueFlush() {
      ...
      await this.app.resources.storage.set("user", refQueue, []);
   }
...
class NetworkRelay extends NetworkRest {
   ...
   async _loadNetworkData() {
      ...
      if (relayState.aesKeySent) return;
