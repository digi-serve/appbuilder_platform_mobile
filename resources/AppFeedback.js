...
class Feedback {
   constructor(options, html2canvasOptions) {
      ...
      this._drawDraw = ($event) => {
         ...
         if (this._state.isDrawing) {
            ...
            if (
               ...
               this._area.width =
                  document.documentElement.scrollWidth - this._area.startX - 4;
            }
            ...
            if (
               ...
               this._area.height =
                  document.documentElement.scrollHeight - this._area.startY - 4;
            }
      ...
      this._highlightElement = ($event) => {
         ...
         if (el) {
            if (
               this._options.allowedTags.indexOf(el.nodeName.toLowerCase()) ===
               -1
            ) {
               this._redraw();
   ...
   _createCheckboxContainer() {
      ...
      checkboxSvgPath.setAttributeNS(
         null,
         "d",
         this._state.includeScreenshot ? this._checkedPath : this._uncheckedPath
      );
         ...
         if (this._state.canDraw && !this._state.isDrawing) {
            ...
            if (!helper.highlight) {
               ...
               this._helpers
                  .filter(
                     (_helper) =>
                        !_helper.highlight && _helper.index !== helper.index
                  )
                  .forEach((_helper) => {
                     this._ctx.fillStyle = "rgba(0,0,0,1)";
                     this._ctx.fillRect(
            ...
            fetch: (url, options) => {
               ...
               return new Promise((resolve, reject) => {
                  ...
                  if (typeof body == "string") {
                     // Network.post() wants the data in object form
                     try {
