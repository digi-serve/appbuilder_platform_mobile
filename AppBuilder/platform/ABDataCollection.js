...
module.exports = class ABDataCollection extends ABDataCollectionCore {
   ...
   async init() {
      ...
      // TODO (Guy): Refactor.
      await this.reset();
      await this.loadData(null, null);
   }

   async loadData(start, limit, waitForSync = true) {
      const dcData = {
         data: [],
         limit: 0,
         offset: 0,
      ...
      if (this._dataStatus === this._dataStatus.initialized) {
         await this.processIncomingData(dcData);
         return;
      }
      if (this._dataStatus === this._dataStatus.initializing) return;
      ...
      const resDcData = (await this.model.findAll(cond)).data;
      let pendingPromises = [];
      for (const key in resDcData) {
         if (key === "data") continue;
         pendingPromises.push(
            storage.set(this.refStorage(), key, resDcData[key].toString())
         );
      }
      await Promise.all(pendingPromises);
      pendingPromises = [];
      for (const value of resDcData.data) {
         pendingPromises.push(
      ...
      await this.processIncomingData(resDcData);
   ...
   async reloadData(start, limit, waitForSync = true) {
      this._dataStatus = this.dataStatusFlag.notInitial;
      await reset();
      await this.loadData(start, limit, waitForSync);
   }
