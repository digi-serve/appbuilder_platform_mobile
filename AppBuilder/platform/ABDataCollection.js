...
module.exports = class ABDataCollection extends ABDataCollectionCore {
   ...
   async _reloadAffectedDC() {
      await Promise.all(
         ...
         ).map((dc) => dc.loadData(null, null, false))
      );
   }
   ...
   async loadData(start, limit, waitForSync = false) {
      ...
      const dcData = (await this.model.findAll(cond)).data;
      let pendingPromises = [];
   ...
   async deleteData(id, waitForSync = false) {
   ...
   async setData(id, value, waitForSync = false) {
   ...
   get model() {
      return (
         this._model ||
         (this._model = (() => {
            // TODO (Guy): Refactor to model() in the future.
