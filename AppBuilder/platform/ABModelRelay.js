	...
	create(values) {
		...
		return Promise.resolve()
			.then(()=>{
				...
				return Network.post(params, responseContext)
				.then(()=>{
...
if (id) {
				...
				return Network['delete'](params, responseContext)
					.then(()=>{
						// a relay doesn't return the data right away so:
						return [];
					})
	...
	console.warn("::: ABModelRelay.delete(): attempting to delete without a valid id:", id);
	...
	findAll(cond) {
		...
		.then(()=>{
			...
			return Network.get(params, responseContext)
			.then(()=>{
				// a relay doesn't return the data right away so:
				return [];
			})
	...
	update(id, values) {
		...
		return Promise.resolve()
			.then(()=>{
				...
				return Network.put(params, responseContext)
					.then(()=>{
						// a relay doesn't return the data right away so:
						return [];
					})
